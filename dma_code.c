...
#include "D:\GiaPhuc\Project_1\TaiLieu\FPGA_Driver.c" // call fpga driver
...
int main() {
    ...
    FILE* fp;
    fp = fopen("bias_img_FC64_hex_0x32b.txt", "r");
    for (int i = 0; i < 10; i++)
    {
        fscanf(fp, "%x", &bias[i]);
        BIAS_DMA[i] = bias[i];
    ...
    FILE* fp;
    fp = fopen("weight_img_FC64_hex_0x32b.txt", "r");
    for (int i = 0; i < 640; i++)
    {
        fscanf(fp, "%x", &weight[i]);
